function [cluster_image] = K_means_Clustering_B(img,opt)
...
%   Detailed explanation goes here
 % specify number of clusters
    ...
    figure; imshow(cluster);title('k-means Clustering Before Cleaning');
    imwrite(cluster,'K-means_clustering_before_cleaning.png');
    cluster1=bwareaopen(cluster,200);
    binIM=cluster1;
    SE=strel('disk',1);
    cluster2=imerode(binIM,SE);
    k=imfill(cluster2,'holes');
    BW = imclose(k,SE);
    cluster3=bwareaopen(BW,100);
